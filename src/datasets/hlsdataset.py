...
class HLSDataset(Dataset):
    ...
    def __init__(self, data_stores: list[xr.Dataset], bands: list[str], mean: list[float] | None = None,
                 std: list[float] | None = None, shape: list[int] | None = None,
        ...
        if self.shape[0] < self.data_shape[0]:
            self.frame_selection = IdxSelection(size=self.shape[0], random=self.data_augmentation)
        else:
    ...
    def preprocess(self, sample: torch.Tensor) -> torch.Tensor:
        ...
        if sample.shape[2] != 1:
            raise ValueError(f"Expected sample to have 1 Timestep, but got {sample.shape[1]}.")
...
def build_hls_dataloader(
    ...
    dataset = HLSDataset(data_stores=data, shape=input_size, bands=bands,
                         scaling=scaling, mean=mean, std=std,
                         data_augmentation=data_aug, chunk_size=chunk_size)
