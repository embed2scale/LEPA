...
class MultimodalTransforms:
    ...
    def __call__(self, data: dict):
        ...
        if hasattr(data[image_modality], "float"):
            data[image_modality] = data[image_modality].float()
...
def get_terramesh_dataloader(
    root_path: str = "https://huggingface.co/datasets/ibm-esa-geospatial/TerraMesh/resolve/main/",
    modalities: list[str] | str = None,
    split: str = "val",
    ...
    if isinstance(modalities, str):
        ...
        normalize = MultimodalNormalize(
            mean={"image": statistics["mean"][modalities[0]]},
            std={"image": statistics["std"][modalities[0]]}
        )
    elif isinstance(modalities, list) and len(modalities) == 1:
        normalize = MultimodalNormalize(
            mean={"image": statistics["mean"][modalities[0]]},
            std={"image": statistics["std"][modalities[0]]}
        )
    ...
    transform = MultimodalTransforms(
        transforms=albumentations.Compose([  # We use albumentations because of the shared transform between image modalities
                Transpose([1, 2, 0]),  # Convert data to channel last (expected shape from albumentations)
                normalize,            
                # albumentations.CenterCrop(*input_size),  # Use center crop in val split
                albumentations.RandomCrop(*input_size),  # Use random crop in train split
                # A.D4(),  # Optionally, use random flipping and rotation for the train split
                albumentations.pytorch.ToTensorV2(),  # Convert to tensor and back to channel first
            ],
